...
int main(int argc, char *argv[])
{
    ...
    if (mode == 0 || mode == 1)
    {
        ...
        double lat = run_kernel_alone(kernel, gridDim, blockDim, kernel_args1, num_runs);

        double lat_sec = lat / 1000;
        double tot_num_bytes = 2 * 1.0 * itrs * num_bytes;            // read and writes
        double bw = (tot_num_bytes / (1024 * 1024 * 1024)) / lat_sec; // GB/s
        cout << "Average achieved bandwidth is " << bw << " GB/S" << endl;
    }
    else if (mode == 2 || mode == 3)
    {
        ...
        {
            cout << "----------------------------------------" << endl;
            cout << "Running two copy kernels colocated using CUDA streams - " << launch_config << endl;
